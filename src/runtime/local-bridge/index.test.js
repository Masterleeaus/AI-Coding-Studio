...
test('exports the browser-safe local bridge contract surface', () => {
  ...
  assert.equal(
    bridge.getCommandDefinition(bridge.COMMANDS.GIT_PUSH).riskLevel,
    bridge.RISK_LEVELS.PUBLISH,
  );
  ...
  assert.equal(bridge.NATIVE_HOST_NAME, 'com.ai_coding_studio.local_bridge');
