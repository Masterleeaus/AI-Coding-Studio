...
> **Private local-first engineering workstation.** AI Coding Studio is evolving from the Better DeepSeek extension into an AI orchestration layer that coordinates existing developer tools. It does not aim to replace VS Code, Git, GitHub CLI, package managers, terminals, build systems, or test runners. Local operating-system work is delegated through an authenticated Local Bridge using allowlisted structured commands.

See [`docs/architecture/LOCAL_AI_DEVELOPMENT_OS.md`](docs/architecture/LOCAL_AI_DEVELOPMENT_OS.md) for the current architecture. Pass 02 implementation evidence is in [`docs/reports/PASS02_ARCHITECTURE_SIMPLIFICATION.md`](docs/reports/PASS02_ARCHITECTURE_SIMPLIFICATION.md). The sections below preserve the working extension feature history while migration proceeds incrementally.
